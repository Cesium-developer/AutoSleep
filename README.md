...
- 启动后台批处理，1 秒后删除 `Uninstall.exe` 自身和空目录
...
- **绿色卸载**：经 Geek Uninstaller 检测，无残留（注册表 64/32 双删 + 自删除）
- **代码透明**：所有逻辑在明文的 `.ps1` 文件中，可用 7-Zip 解包查看
- **唤醒恢复**：唤醒后自动重置计时器，避免误触发
...
1. 双击 `AutoSleep_Setup.exe`（管理员权限自动请求）
2. 安装程序会自动完成：
...
不会。打印作业通常只有几秒到几十秒，只要你的空闲等待时间大于 5 分钟，打印不会影响计时器。
